<script>
  ...
  async function loadPages() {
    ...
    loading = true;
    try {
      pagesResult = await getComparePages(
        sessionA,
        sessionB,
        pagesDiffType,
        PAGE_SIZE,
  ...
  async function loadLinks() {
    ...
    loading = true;
    try {
      linksResult = await getCompareLinks(
        sessionA,
        sessionB,
        linksDiffType,
        PAGE_SIZE,
  ...
  <div class="compare-selectors">
    <div class="selector-group">
      ...
      <SearchSelect
        ...
        options={[
          { value: '', label: t('compare.selectSession') },
          ...sessions.map((s) => ({ value: s.ID, label: sessionLabel(s) })),
        ]}
        onsearch={sessions.length > 20
          ? async (q) => {
              const lq = q.toLowerCase();
              return sessions
                .filter((s) => sessionLabel(s).toLowerCase().includes(lq))
                .map((s) => ({ value: s.ID, label: sessionLabel(s) }));
            }
          : undefined}
      />
    </div>
  ...
  {#if compareStats}
    <div class="tab-bar compare-tab-bar">
      <button
        ...
        onclick={() => switchMainTab('stats')}>{t('compare.stats')}</button
      >
      <button
        class="tab"
        class:tab-active={activeTab === 'pages'}
        onclick={() => switchMainTab('pages')}>{t('common.pages')}</button
      >
      <button
        class="tab"
        class:tab-active={activeTab === 'links'}
        onclick={() => switchMainTab('links')}>{t('compare.internalLinks')}</button
      >
    </div>
