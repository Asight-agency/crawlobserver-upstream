...
{#if data}
  ...
  <table>
    <thead>
      <tr
        ><th>#</th><th>{t('common.url')}</th><th>{t('urlDetail.pageRank')}</th><th
          >{t('urlDetail.depth')}</th
        ><th>{t('pagerank.intLinks')}</th><th>{t('pagerank.extLinks')}</th><th
          >{t('session.words')}</th
        ><th>{t('common.status')}</th><th>{t('session.title')}</th></tr
      >
    </thead>
    <tbody>
      {#each data.pages || [] as p, i}
        <tr>
          ...
          <td class="cell-url"
            ><a href="#detail" onclick={(e) => goToUrlDetail(e, p.url)}>{p.url}</a></td
          >
